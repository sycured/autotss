...
class autotss:
    ...
    def checkForBlobs(self, deviceECID, buildID):
        ...
        for entry in loads(deviceInfo['blobsSaved']):
    ...
    def getFirmwareAPI(self):
        ...
        rawResponse = get('https://api.ipsw.me/v2.1/firmwares.json/condensed', headers=headers)
    ...
    def saveBlobs(self, device, buildID, versionNumber):
        ...
        tssCall = Popen(scriptArguments, stdout=PIPE)

        tssOutput = []
        for line in TextIOWrapper(tssCall.stdout, encoding='utf-8'):
        ...
        if 'Saved shsh blobs!' in tssOutput:
            ...
            print('[{0}] [{1} - {2}] {3}'.format(device['deviceName'], versionNumber, buildID,
                                                 'Error, see log file: ' + savePath + '/tsschecker_log.txt'))
    ...
    def getScriptPath(self, userPath):
        ...
        try:
            tssCall = Popen(scriptPath, stdout=PIPE)
